...
Write-Host "Do you want to modify the Control Plane Count AKSCluster: $AKSCluster ?" -ForegroundColor Green
Write-Host "1. Yes                                                            " 
Write-Host "2. No                                                             " 
$giveMeNumber = {
      try {
    ...
    while ($null -eq $option -or $option -lt 1 -or $option -gt 2) {
     ...
     switch ($option) {
      1 {
        $AKSCPCount = $(az aksarc show --name $AKSCluster --resource-group $resource_group --query "properties.controlPlane.count")
        Write-Host "Actual Control Plane Count at $AKSCluster : $AKSCPCount"

        do {
            [int]$AKSCPCountnew = Read-Host "How many Control Plane Count do you need?"

            # Check if the AKSCPCount is odd
            if ($AKSCPCountnew % 2 -eq 0) {
                Write-Host "Error: ControlPlane replicas should be odd in number. Please enter an odd number."
            } else {
                az aksarc update --name $AKSCluster --resource-group $resource_group --control-plane-node-count $AKSCPCountnew
                break
            }
} while ($true)
      }
      2 { 
        Write-Host "AKSCluster $AKSCluster was Control Plane was not modified"
        Start-Sleep 1
      }
      }
#endregion
