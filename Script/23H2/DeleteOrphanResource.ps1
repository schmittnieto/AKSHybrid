...
Write-Host "Select the Subscription" -ForegroundColor Green
...
Write-Host "Select the Resource" -ForegroundColor Green
$ResourceSelector = Get-Option-Az $(az resource list -g $resource_group --output json) "name"
$ResourceID = $(az resource list --resource-group $resource_group --query "[?name=='$ResourceSelector'].id" --output tsv)
Write-Host "Do you want to delete the Resource: $ResourceID ?" -ForegroundColor Green
Write-Host "1. Yes                                                            " 
Write-Host "2. No                                                             " 
$giveMeNumber = {
      try {
    ...
    while ($null -eq $option -or $option -lt 1 -or $option -gt 2) {
     ...
     switch ($option) {
      1 {
        az resource delete --ids $resourceId --latest-include-preview --debug
        Write-Host "The resource $ResourceSelector was deleted"
        Start-Sleep 10
      }
      2 { 
        Write-Host "The resource $ResourceSelector was not deleted"
        Start-Sleep 1
      }
      }
#endregion